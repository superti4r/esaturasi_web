...
if (isset($_POST['cari'])) {
  ...
  $sql = mysqli_query($koneksi, "SELECT * FROM vposttest WHERE kd_bab_kelas = '$kd_bab_kelas' ORDER BY kd_posttest ASC");
} else {
  $sql = mysqli_query($koneksi, "SELECT * FROM vposttest WHERE kd_bab_kelas = '$kd_bab_kelas' ORDER BY kd_posttest ASC");
}
...
<section class="section">
  <div class="section-header d-flex justify-content-between align-items-center">
    <h1>Posttest</h1>
    <div> 
      <a href="createposttest.php?kode_mpp=<?php echo urlencode($kode_mpp); ?>&kd_mapel=<?php echo urlencode($kd_mapel); ?>&kd_bab_kelas=<?php echo urlencode($kd_bab_kelas); ?>" class="btn btn-primary mr-2">Tambah Posttest</a>
            ...
            <table class="table table-hover table-striped w-100" id="table-1">
              ...
              <tbody id="tableData">
                ...
                <tr>
                  ...
                  <td><?php echo $data['judul_posttest']; ?></td>
...
<td><?php echo date('d-m-Y H:i', strtotime($data['selesai_pengerjaan'])); ?></td>
                  <td>
                  <a href="daftar_soal_posttest.php?kd_posttest=<?php echo $data['kd_posttest']; ?>"><button class="btn btn-success btn-sm"><i class="fas fa-plus fa-fw"></i></button></a>
                    <a href="hapusposttest.php?kd_posttest=<?php echo $data['kd_posttest']; ?>&pesan=hapus" onClick="return confirm('Apakah data yang Anda pilih akan dihapus?')">
