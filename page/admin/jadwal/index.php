...
$katakunci = "";
if (isset($_POST['cari'])) {
  $katakunci = mysqli_real_escape_string($koneksi, $_POST['kata_kunci']);
}
        ...
        if (mysqli_num_rows($sql_kelas) > 0) {
          while ($kelas = mysqli_fetch_assoc($sql_kelas)) {
            ...
            foreach ($hari as $h) {
              ...
              $h_escaped = mysqli_real_escape_string($koneksi, $h); // Escaping hari
              $kd_kelas_escaped = mysqli_real_escape_string($koneksi, $kd_kelas); // Escaping kode kelas
              $sql_jadwal = mysqli_query($koneksi, "
                  SELECT * FROM vjadwal
                  WHERE kd_kelas = '$kd_kelas_escaped' AND hari = '$h_escaped' AND status = 'aktif'
